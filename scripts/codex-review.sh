...
# 【錨在嚴重度上,不是清單前綴】(2026-09-07 codex 複查抓到,第一版修正沒補乾淨):
# 先前改成「清單前綴 + 標註」仍有兩個洞,兩個都實測過:
#   - `- 標註說明: 【已讀】=已查證` —— legend 只要自己帶清單前綴就又被算成發現 → 漏報
#   - `### Required【推論】`        —— 標題式發現沒有清單前綴,兩個計數都是 0 → 漏報
# 真正的不變量是「一則發現必然帶嚴重度標籤」(prompt 的輸出格式行強制要求),legend 與自白
# 散文都不會帶。所以錨嚴重度、放寬呈現格式(清單/編號/標題/粗體都涵蓋)。
# `[^【]{0,80}` 限制嚴重度與標註之間的距離,避免散文裡偶然提到 Required 又在遠處出現標註
# 而誤判;80 是量出來的——最長的實際寫法是「- Critical — `path/to/file.go:57-58`【已讀】」。
