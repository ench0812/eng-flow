...
BEGIN {
    FS = US
    nm = 0; nl = 0
    # 拆分候選門檻。2026-08-22 由 2048 調到 3072：實測 10 則真實記憶中有 7 則
    # 落在 2048 之上，而其中多數是「一個主題寫得完整」而不是「塞了好幾件事」。
    # 建議清單長到看不完就沒有人會看，等於這項檢查失效。
    # 代價講清楚：2.5KB 上下的多主題記憶從此不會被提醒，那部分改由人判斷。
    SPLIT_BYTES = 3072      # 拆分候選門檻
    SPLIT_PARAS = 4
