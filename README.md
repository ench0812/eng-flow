...
**PostgreSQL memory backend — moved out of this repo (2026-09-07).** It now lives in the private `claude-memory-pg` repo. It was never part of the plugin surface: no skill, hook, or manifest entry referenced it, and nothing here imports it — it shared this repo only because that is where it was first written. The split keeps this public repo to the plugin itself, and keeps the backend's test fixtures (which carry real project slugs) out of public view. Runtime is unaffected either way: `~/.claude/scripts/memory` resolves the installed venv at `~/.claude/memory-pg/.venv/`, never the source tree, so only the "where do I edit and reinstall from" answer changed.
