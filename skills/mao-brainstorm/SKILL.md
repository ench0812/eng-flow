...
**為什麼是這一步**：方案選錯**沒有快速的驗證迴路**——要等實作完才知道，而那時改架構的代價遠高於此刻多想一輪。判準與成本說明的完整版在 `references/model-routing.md` 的 D 層段落。
...
**Brief 給目標，不要給模板。** Fable 5.1 對過度規定的 prompt 反而產出更差，所以不要寫輸出格式或行數上限（其他 tier 的 150 行限制不適用於 D 層）。給它：已釐清的需求與約束（Clarify 的**結論**，不是逐輪對話）、要動的模組**路徑**（讓它自己讀，不要貼程式碼）、明確的 out-of-scope，以及一句目標：**找出這個問題值得考慮的解法，並說明你推薦哪一個。**

**自我檢查用的完整性清單**——不是輸出格式，**順序也不代表章節順序**：

- 有哪些**實質不同**的走法？各自怎麼做、代價是什麼、什麼情況下它才是對的？
- 推薦哪一個、為什麼——尤其是它在哪些條件下會變成錯的選擇？
- 推薦方案的架構長什麼樣：元件邊界、資料流、錯誤路徑、狀態轉換、怎麼測？
- 有哪些**它無法自己決定、需要使用者裁決**的取捨？

不要求它寫成 spec 文字——成文是第 5 步的事，由主線做。

**你（Opus）接手**：先自己審一遍（不是照抄——它沒參與 Clarify 的對話，可能誤解了某個約束），再進第 3 步向使用者呈現。第 4、5 步的成文與逐節確認由你做，第 6 步的 codex 共議是你與 codex 兩方，**Fable 不參與後續任何一輪**，也不看 codex 的回覆。

**兩個硬限制**（詳見 `references/model-routing.md`）：Agent tool 沒有 `effort` 參數，派工的 effort 繼承 session；Fable 5.1 在 ZDR 組織不可用，會回 400。**Fable 不可用時**直接由你自己走第 3 步，交付時講一句「本次未經 Fable 起草」——這是加速器不是必要條件。
