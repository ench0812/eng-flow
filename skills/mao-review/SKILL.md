...
Set `<level>` to the **highest original severity this review assigned** to the change (Critical/Required/Optional/Nit/FYI — even if now fixed; the risk area remains). The script maps severity → model/effort; use `scripts/codex-review.sh` as the executable source of truth and `references/model-routing.md` for its rationale. Do not duplicate the model table here. Severity is your input from this review — never let the script re-triage it. Over-estimate when unsure (an omitted or unknown severity lands on the lowest tier, the same model as `optional` — anything that might matter belongs at `required` or above).
...
**Cost discipline.** The script keeps a per-review-line round ledger and prints a **non-blocking** warning from the 3rd consultation on the same base: `[codex-review] 警示: 這條複查線(diff:<base>)本次是第 N 次諮詢`. That warning means you are consulting per fix instead of per round — the correct response is to finish **all** of this round's Required/Critical fixes and consult once, not to consult again immediately. Two related behaviours: identical content twice in a row is refused outright (`SKIP: 送出內容與上一輪...完全相同`), session resume exists but is **off by default** (measured: the server-side cache window is only tens of seconds, so at a normal review cadence resuming costs more than a fresh call — see `references/model-routing.md`). Token usage per call, including cache hit rate, is appended to `$CODEX_REVIEW_LOG`; `scripts/codex-usage.sh` reports it.

**Zero-verification:** if the script answers `[codex-review] 警示: 本輪【零查證】`, the consultation happened but codex read **no files** — every finding is inference from the diff alone, and the script still exits 0 with a normal `完成(...)` line. Do **not** report this as a completed codex review. Present the findings as **hypotheses to verify**, check each one against the actual code yourself, and say in one line that this round was unverified. The usual cause is a path problem, not a real permission problem — the script's own note says so. (Known gap: "read nothing *and* found nothing" does not trigger this warning.)
